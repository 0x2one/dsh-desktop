 ...
 * 2. The window-controls toolbar and its buttons must be transparent
 *    (background: transparent / rgba(0,0,0,0)), so the controls float over
 *    the content without painting a strip of their own.
 *
 * Boots dsh web (temp DSH_HOME + injected plugin), opens it in the installed
 * chromium, and asserts both invariants.
 *
...
try {
  ...
  if (visible > 0) {
    ...
    // 2. Toolbar + button backgrounds must be transparent.
  ...
  if (visible === 0 || !heroOk || !toolbarTransparent || !buttonTransparent) {
    console.error('FAIL: hero layout invariants not met')
    process.exitCode = 1
  } else {
    console.log('PASS: hero content reaches the top and the controls are transparent')
  }
} catch (error) {
