...
- **设置「桌面」分区**（`plugins/dsh-desktop-settings/`）：向 harness `settings.section` 注册独立页（order 5，插在「通用」与「模型」之间），提供快捷键 / 开机自启 / 启动环境 / 检查更新。快捷键录制、新增环境和检查更新都在设置页内嵌完成；经 `window.api.desktop` / `window.api.updater` 调用主进程已有实现，改完后 `syncDesktopChrome` 刷新托盘并广播 snapshot。托盘菜单只保留显示窗口、开机自启、切换已有环境和退出。启动静默检查发现新版本时只广播状态，不弹窗；设置页用白名单解析渲染 GitHub Release 的 HTML / Markdown。
