...
**注意：** 本版本将嵌入的 `@deepseek-ai/dsh` 升级到 `0.1.5-rc.2`。这次更新可能造成启动失败；若遇到无法启动，请手动删除 `~/.dsh/profiles/` 中对应环境目录（默认 `dsh-desktop`）后重试。

## 功能

- 嵌入固定版本 `@deepseek-ai/dsh@0.1.5-rc.2`（`npx` 子进程，独立 Node ABI，不打进 Electron）
...
- 开机自启：托盘「开机自启 → 开启 / 关闭」；自启时静默启动到托盘，不弹主窗口
- 窗口状态记忆：记住窗口大小 / 位置 / 最大化状态，重启后恢复
- 首次关闭到托盘时弹出一次系统通知，说明如何恢复窗口与真正退出
...
首次启动会联网拉取 `@deepseek-ai/dsh@0.1.5-rc.2`；若当前 profile 尚未安装 `dshmarket`，还会阻塞安装插件市场（可能数分钟）。之后与本地 `dsh web` 共用同一套依赖，一般不再重复下载。
...
macOS 产物未签名未公证，首次打开时系统会拦截：在 Finder 中右键应用图标 →「打开」，或到「系统设置 → 隐私与安全性」点击「仍要打开」。安装后启动数秒会静默检查更新；发现新版本后到设置「桌面」分区查看说明并下载。下载完成可立即重启安装，或等到托盘「退出」时安装。也可随时在设置里手动检查。`pnpm dev` 开发模式不访问更新源。
...
- **显示窗口** — 恢复主窗口
- **开机自启** — 二级菜单：开启 / 关闭；开启后随系统登录静默进托盘
- **启动环境** — 列出 `~/.dsh/profiles/` 下的 profile，单选切换；上次选择会记住
- **退出** — 真正退出应用

快捷键、新增环境和检查更新在设置「桌面」分区完成。默认环境名为 `dsh-desktop`（目录 `~/.dsh/profiles/dsh-desktop`）。切换或新建环境只影响桌面端当前会话，不会改写用户自己的 `web` profile。
...
Electron 主进程          dsh web（独立 Node 子进程）
  环境检查 / 托盘          npx @deepseek-ai/dsh@0.1.5-rc.2
